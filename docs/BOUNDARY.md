...
- 发布形态是当前用户目录下的固定 `bin/` + `obs/` 布局；升级覆盖同一路径，不依赖
  Cargo `target` 或开发机环境变量，也不把 OBS 源码树装入产品目录。
...
编码器中选择，并在硬编 start 失败时明文降级至 x264。M5 增加 OBS 窗口捕获以及
WASAPI 麦克风与系统声混音。M6 增加单显示器区域裁剪、多显示器选择、OBS 原生暂停
和 Windows 全局热键。M7 以 Windows 原生覆盖层作为区域框选主路径，并接入真实 OBS
`game_capture`；它不会以普通窗口捕获伪装成功。M8 增加带明确 32×32 黑色兼容占位轨的
纯音频模式、DX11 回归靶和发行冒烟；摄像头继续不在当前边界内。
